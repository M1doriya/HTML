...
def get_company_name(data):
    """Robust company name extraction across schema variants and imperfect JSON."""
    # Primary: v5+ location
    ...
    candidates = [
        ...
        data.get('company'),
        (data.get('metadata') or {}).get('company_name'),
    ]
    # Fallback: sometimes embedded in first account name/holder field
    accounts = data.get('accounts') or []
    if accounts:
        a0 = accounts[0] or {}
        candidates.extend([a0.get('account_name'), a0.get('account_holder'), a0.get('holder')])
    # Pick first non-empty string-ish value
    for c in candidates:
        if isinstance(c, str):
            cleaned = ' '.join(c.split()).strip()
            if cleaned and cleaned.lower() not in {'unknown', 'n/a', 'na', 'null', 'none', '-'}:
                return cleaned
    return 'Unknown'
...
def generate_interactive_html(data):
    ...
    company = get_company_name(data) or 'Company'
    ...
    if counterparties:
        ...
        # NOTE: build optional 'both sides' section outside f-string to avoid nested triple-quote tokenization issues
        both_sides_section = ""
        if parties_both:
            both_sides_section = f'''<div class="flag-card" style="margin-top:1.5rem">
    ...
    # NOTE: build optional inter-account transfers section outside f-string to avoid nested triple-quote tokenization issues
    inter_transfers_section = ""
    if inter_transfers:
        inter_transfers_section = f'''<div class="section" style="margin-top:1.5rem">
...
st.caption("Backwards compatible with v4.0 schema | All validation issues fixed")

uploaded_file = st.file_uploader("Upload JSON Analysis Output", type=['json'])
...
if data:
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.download_button("📄 DOWNLOAD JSON", json.dumps(data, indent=2), f"{company.replace(' ','_')}_analysis.json", "application/json", use_container_width=True)
    with c2:
        st.download_button("🌐 DOWNLOAD INTERACTIVE HTML", generate_interactive_html(data), f"{company.replace(' ','_')}_analysis.html", "text/html", use_container_width=True)
    
    st.markdown("---")
    
    # Simple Streamlit preview
    r = data.get('report_info', {})
    ...
    # v5.0 specific metrics
    if schema_version == '5.0':
        st.markdown("#### v5.0 Features")
        c1, c2, c3 = st.columns(3)
        
        returned = data.get('flags', {}).get('returned_cheques', {})
        c1.metric("Returned Cheques", returned.get('count', 0) if isinstance(returned, dict) else 0)
        
        recurring = data.get('recurring_payments', {})
        if recurring:
            # FIX: assessment is a STRING in the schema, not a dict
            # Schema: "assessment": "OK - Statutory payments regular"
            assessment = recurring.get('assessment', 'N/A')
            # Display the string directly, or extract status from it
            if isinstance(assessment, str):
                # Show OK/ALERT based on the assessment text
                status_display = "OK" if "OK" in assessment.upper() else ("ALERT" if "ALERT" in assessment.upper() else assessment[:20])
                c2.metric("Statutory Compliance", status_display)
            elif isinstance(assessment, dict):
                c2.metric("Statutory Compliance", assessment.get('statutory_compliance', 'N/A'))
            else:
                c2.metric("Statutory Compliance", 'N/A')
        
        nonbank = data.get('non_bank_financing', {})
        if nonbank:
            # FIX: risk_level is a direct field in non_bank_financing, not inside assessment
            # Schema: "risk_level": "LOW", "assessment": "No suspected unlicensed..."
            risk_level = nonbank.get('risk_level', 'N/A')
            c3.metric("Non-Bank Risk", risk_level)
    ...
    st.info("👆 Upload a JSON file to view and download the analysis")
